...
'''
    ...
    tracker.record_tool_call(
        "write_file",
        {"path": str(project_dir / "debug.log"), "content": "DEBUG: add(2, 3) = 5\nDEBUG: multiply(4, 5) = 20\n"},
        result="ok",
        duration_ms=30
    )
    tracker.record_tool_call(
        "write_file",
        {"path": str(project_dir / "config.json"), "content": json.dumps({
            "name": "calculator-demo",
            "version": "1.0.0",
            "debug": True,
            "log_level": "DEBUG"
...
def run_demo(show_json: bool = False) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"   Recorded {trajectory.total_steps} steps with {trajectory.total_tool_calls} tool calls")
        ...
        for step in trajectory.steps:
            ...
            for tc in step.tool_calls:
                ...
                print(f"    {status} {tc.name}({list(tc.arguments.keys())}) → {tc.duration_ms:.0f}ms")
        
        # Save trajectory for inspection
        traj_path = Path(tmpdir) / "trajectory.json"
